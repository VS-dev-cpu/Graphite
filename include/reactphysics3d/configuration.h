...
/// NON_LINEAR_GAUSS_SEIDEL : Slower but more precise. This is the option used
/// by default.
...
/// bounciness
///                      in some situations (due to error correction factor
///                      being added to the bodies momentum).
/// SPLIT_IMPULSES : A bit slower but the error correction factor is not added
/// to the
...
/// inflated by a constant percentage of its size to allow the collision shape
/// to move a little bit without triggering a large modification of the tree
/// each frame which can be costly
...
constexpr uint8 NB_MAX_POTENTIAL_CONTACT_MANIFOLDS =
    4 * NB_MAX_CONTACT_MANIFOLDS;
...
/// Distance threshold to consider that two contact points in a manifold are the
/// same
